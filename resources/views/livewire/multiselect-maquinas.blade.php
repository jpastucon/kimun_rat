<div wire:init="loadMultiselect">

    <div class="w-full">
        <select x-cloak id="select" class="hidden">
            @foreach ($maquinas as $maquina)
                <option class="py-1 " value="{{ $maquina->id }}">{{ $maquina->name }} - {{ $maquina->namemarca }} - {{ $maquina->namemodelo }}</option>
            @endforeach
        </select>
        <div x-data="dropdownBis({{ json_encode($maquinas) }})" x-init="loadOptionsBis()" class="w-full flex flex-col items-center sh-64 mx-auto">
            <input name="values" id="values" value="" type="hidden" x-bind:value="selectedValuesBis()">
                        ...
                        <div class="my-2 p-1 flex border border-gray-200 bg-white rounded">
                            <div class="flex flex-auto flex-wrap">
                                <template x-for="(option,index) in selected" :key="options[option].value">
                                    <div
                                        ...
                                        <div class="flex flex-auto flex-row-reverse">
                                            <div @click.close="removeBis(index,option)">
                                ...
                                <div x-show="selected.length == 0" class="flex-1">
                                    <input placeholder="Seleccione la/s maquina/s"
