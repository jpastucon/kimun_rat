    ...
    <div class="w-full">
        <select x-cloak id="select" class="hidden">
            @foreach ($tipos as $tipo)
                <option class="py-1 " value="{{ $tipo->id }}">{{ $tipo->name }}</option>
            @endforeach
        </select>
        <div x-data="dropdownBis({{ json_encode($tipos) }})" x-init="loadOptionsBis()" class="w-full flex flex-col items-center sh-64 mx-auto">
